...
!!!143490.php!!!	addCode(in name : string, in callback_type : string, in callback_func : string, in callback_params : array, in content_type : string, in allowed_within : array, in not_allowed_within : array) : bool
		...
		return true;
!!!143618.php!!!	removeCode(in name : $name) : bool
		if (isset ($this->_codes[$name])) {
			unset ($this->_codes[$name]);
			return true;
		}
		return false;
!!!143746.php!!!	removeAllCodes()
		$this->_codes = array ();
!!!143874.php!!!	setCodeFlag(in name : string, in flag : string, in value : mixed) : bool
		if (!isset ($this->_codes[$name])) {
			return false;
		}
		$this->_codes[$name]['flags'][$flag] = $value;
		return true;
!!!144002.php!!!	setOccurrenceType(in code : string, in type : string) : bool
		return $this->setCodeFlag ($code, 'occurrence_type', $type);
!!!144130.php!!!	setMaxOccurrences(in type : string, in count : int) : bool
		...
		return true;
!!!144258.php!!!	addParser(in type : string, in parser : mixed) : bool
		if (is_array ($type)) {
			foreach ($type as $t) {
		...
		return true;
!!!144386.php!!!	setRootContentType(in content_type : string)
		$this->_rootContentType = $content_type;
!!!144514.php!!!	setRootParagraphHandling(in enabled : bool)
		$this->_rootParagraphHandling = (bool)$enabled;
!!!144642.php!!!	setParagraphHandlingParameters(in detect_string : string, in start_tag : string, in end_tag : string)
		$this->_paragraphHandling = array (
			'detect_string' => $detect_string,
			'start_tag' => $start_tag,
			'end_tag' => $end_tag
		);
!!!144770.php!!!	setGlobalCaseSensitive(in caseSensitive : bool)
		$this->_caseSensitive = (bool)$caseSensitive;
!!!144898.php!!!	globalCaseSensitive() : bool
		return $this->_caseSensitive;
!!!145026.php!!!	setMixedAttributeTypes(in mixedAttributeTypes : bool)
		$this->_mixedAttributeTypes = (bool)$mixedAttributeTypes;
!!!145154.php!!!	mixedAttributeTypes() : bool
		return $this->_mixedAttributeTypes;
!!!145282.php!!!	setValidateAgain(in validateAgain : bool)
		$this->_validateAgain = (bool)$validateAgain;
!!!145410.php!!!	validateAgain() : bool
		return $this->_validateAgain;
!!!145538.php!!!	getCodeFlag(in name : string, in flag : string, in type : string = 'mixed', in default : mixed = null) : bool
		if (!isset ($this->_codes[$name])) {
			return $default;
		}
		...
		return $return;
!!!145666.php!!!	_setStatus(in status : )
		switch ($status) {
			case 0:
		...
		return true;
!!!145794.php!!!	_appendText(in text : string) : bool
		if (!strlen ($text)) {
			return true;
		}
		switch ($this->_status) {
...
!!!145922.php!!!	_reparseAfterCurrentBlock() : bool
		if ($this->_status == 2) {
...
!!!146050.php!!!	_applyParsers(in type : , in text : )
		if (!isset ($this->_parsers[$type])) {
			return $text;
		}
		...
		return $text;
!!!146178.php!!!	_handleStatus(in status : int, in needle : string) : bool
		switch ($status) {
			case 0: // NORMAL TEXT
				if ($needle != '[' && $needle != '[/') {
		...
		return true;
!!!146306.php!!!	_openElement(in type :  = 0) : bool
		...
		return true;
!!!146434.php!!!	_isCloseable(in name : , inout closecount : ) : bool
		...
		return false;
!!!146562.php!!!	_validateCloseTags(in closecount : ) : bool
		$scount = count ($this->_stack);
		...
		return true;
!!!146690.php!!!	_isOpenable(in name : , inout closecount : ) : bool
		if (!isset ($this->_codes[$name])) {
			return false;
		}
		...
		return true;
!!!146818.php!!!	_isOpenableWithClose(in name : , inout closecount : ) : bool
		...
		return false;
!!!146946.php!!!	_closeRemainingBlocks()
		// everything closed
		if (count ($this->_stack) == 1) {
			return true;
		}
		...
		return true;
!!!147074.php!!!	_findNamedNode(in name : , in searchdeeper :  = false) : mixed
...
!!!147202.php!!!	_outputTree() : bool
		...
		return false;
!!!147330.php!!!	_outputNode(inout node : ) : bool
		$output = '';
...
!!!147458.php!!!	_modifyTree() : bool
		...
		return true;
!!!147586.php!!!	_handleParagraphs(inout node : object) : bool
		...
		unset ($paragraph);
		return true;
!!!147714.php!!!	_hasParagraphAncestor(inout node : object) : bool
		if ($node->_parent === null) {
			return false;
		}
		$parent = $node->_parent;
...
!!!147842.php!!!	_breakupNodeByParagraphs(inout node : object) : array
		...
		$dest_nodes[] = $dest_node;
		return $dest_nodes;
!!!147970.php!!!	_isUseContent(inout node : object, in check_attrs : bool = false) : bool
		...
		return !$result;
!!!148098.php!!!	_getCanonicalName(in name : string) : string
		if (isset ($this->_codes[$name])) {
			return $name;
		}
		$found = false;
