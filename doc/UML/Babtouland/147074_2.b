...
!!!248322.php!!!	addCode(in name : , in callback_type : , in callback_func : , in callback_params : , in content_type : , in allowed_within : , in not_allowed_within : )
		...
		return true;
!!!248450.php!!!	removeCode(in name : )
		if (isset ($this->_codes[$name])) {
			unset ($this->_codes[$name]);
			return true;
		}
		return false;
!!!248578.php!!!	removeAllCodes()
		$this->_codes = array ();
!!!248706.php!!!	setCodeFlag(in name : , in flag : , in value : )
		if (!isset ($this->_codes[$name])) {
			return false;
		}
		$this->_codes[$name]['flags'][$flag] = $value;
		return true;
!!!248834.php!!!	setOccurrenceType(in code : , in type : )
		return $this->setCodeFlag ($code, 'occurrence_type', $type);
!!!248962.php!!!	setMaxOccurrences(in type : , in count : )
		...
		return true;
!!!249090.php!!!	addParser(in type : , in parser : )
		if (is_array ($type)) {
			foreach ($type as $t) {
		...
		return true;
!!!249218.php!!!	setRootContentType(in content_type : )
		$this->_rootContentType = $content_type;
!!!249346.php!!!	setRootParagraphHandling(in enabled : )
		$this->_rootParagraphHandling = (bool)$enabled;
!!!249474.php!!!	setParagraphHandlingParameters(in detect_string : , in start_tag : , in end_tag : )
		$this->_paragraphHandling = array (
			'detect_string' => $detect_string,
			'start_tag' => $start_tag,
			'end_tag' => $end_tag
		);
!!!249602.php!!!	setGlobalCaseSensitive(in caseSensitive : )
		$this->_caseSensitive = (bool)$caseSensitive;
!!!249730.php!!!	globalCaseSensitive()
		return $this->_caseSensitive;
!!!249858.php!!!	setMixedAttributeTypes(in mixedAttributeTypes : )
		$this->_mixedAttributeTypes = (bool)$mixedAttributeTypes;
!!!249986.php!!!	mixedAttributeTypes()
		return $this->_mixedAttributeTypes;
!!!250114.php!!!	setValidateAgain(in validateAgain : )
		$this->_validateAgain = (bool)$validateAgain;
!!!250242.php!!!	validateAgain()
		return $this->_validateAgain;
!!!250370.php!!!	getCodeFlag(in name : , in flag : , in type :  = 'mixed', in default :  = null)
		if (!isset ($this->_codes[$name])) {
			return $default;
		}
		...
		return $return;
!!!250498.php!!!	_setStatus(in status : )
		switch ($status) {
			case 0:
		...
		return true;
!!!250626.php!!!	_appendText(in text : )
		if (!strlen ($text)) {
			return true;
		}
		switch ($this->_status) {
...
!!!250754.php!!!	_reparseAfterCurrentBlock()
		if ($this->_status == 2) {
...
!!!250882.php!!!	_applyParsers(in type : , in text : )
		if (!isset ($this->_parsers[$type])) {
			return $text;
		}
		...
		return $text;
!!!251010.php!!!	_handleStatus(in status : , in needle : )
		switch ($status) {
			case 0: // NORMAL TEXT
				if ($needle != '[' && $needle != '[/') {
		...
		return true;
!!!251138.php!!!	_openElement(in type :  = 0)
		...
		return true;
!!!251266.php!!!	_isCloseable(in name : , inout closecount : )
		...
		return false;
!!!251394.php!!!	_validateCloseTags(in closecount : )
		$scount = count ($this->_stack);
		...
		return true;
!!!251522.php!!!	_isOpenable(in name : , inout closecount : )
		if (!isset ($this->_codes[$name])) {
			return false;
		}
		...
		return true;
!!!251650.php!!!	_isOpenableWithClose(in name : , inout closecount : )
		...
		return false;
!!!251778.php!!!	_closeRemainingBlocks()
		// everything closed
		if (count ($this->_stack) == 1) {
			return true;
		}
		...
		return true;
!!!251906.php!!!	_findNamedNode(in name : , in searchdeeper :  = false)
...
!!!252034.php!!!	_outputTree()
		...
		return false;
!!!252162.php!!!	_outputNode(inout node : )
		$output = '';
...
!!!252290.php!!!	_modifyTree()
		...
		return true;
!!!252418.php!!!	_handleParagraphs(inout node : )
		...
		unset ($paragraph);
		return true;
!!!252546.php!!!	_hasParagraphAncestor(inout node : )
		if ($node->_parent === null) {
			return false;
		}
		$parent = $node->_parent;
...
!!!252674.php!!!	_breakupNodeByParagraphs(inout node : )
		...
		$dest_nodes[] = $dest_node;
		return $dest_nodes;
!!!252802.php!!!	_isUseContent(inout node : , in check_attrs :  = false)
		...
		return !$result;
!!!252930.php!!!	_getCanonicalName(in name : )
		if (isset ($this->_codes[$name])) {
			return $name;
		}
		$found = false;
