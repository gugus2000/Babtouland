...
!!!245250.php!!!	__construct(in occurredAt :  = -1)
		$this->_id = $GLOBALS['__STRINGPARSER_NODE_ID']++;
		$this->occurredAt = $occurredAt;
!!!245378.php!!!	type()
		return $this->_type;
!!!245506.php!!!	prependChild(inout node : )
		if (!is_object ($node)) {
			return false;
		}
		...
		return true;
!!!245634.php!!!	appendToLastTextChild(in text : )
...
!!!245762.php!!!	appendChild(inout node : )
		if (!is_object ($node)) {
			return false;
		}
		...
		$node->_parent = $this;
		return true;
!!!245890.php!!!	insertChildBefore(inout node : , inout reference : )
		if (!is_object ($node)) {
			return false;
		}
		...
		return true;
!!!246018.php!!!	insertChildAfter(inout node : , inout reference : )
		if (!is_object ($node)) {
			return false;
		}
		...
		return true;
!!!246146.php!!!	removeChild(inout child : , in destroy :  = false)
		if (is_object ($child)) {
			// if object: get index
			$object = $child;
			unset ($child);
		...
		return true;
!!!246274.php!!!	firstChild()
		$ret = null;
		if (!count ($this->_children)) {
			return $ret;
		}
		return $this->_children[0];
!!!246402.php!!!	lastChild()
		$ret = null;
...
!!!246530.php!!!	destroyNode(inout node : )
		if ($node === null) {
			return false;
		}
...
!!!246658.php!!!	_destroy()
		return true;
!!!246786.php!!!	_findChild(inout child : )
		if (!is_object ($child)) {
			return false;
		}
		...
		return false;
!!!246914.php!!!	equals(inout node : )
		return ($this->_id == $node->_id);
!!!247042.php!!!	matchesCriterium(in criterium : , in value : )
		return false;
!!!247170.php!!!	getNodesByCriterium(in criterium : , in value : )
		$nodes = array ();
		$node_ctr = 0;
		...
		return $nodes;
!!!247298.php!!!	getNodeCountByCriterium(in criterium : , in value : )
		$node_ctr = 0;
		for ($i = 0; $i < count ($this->_children); $i++) {
			if ($this->_children[$i]->matchesCriterium ($criterium, $value)) {
				$node_ctr++;
